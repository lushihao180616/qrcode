...
public class BeanCostServiceImpl implements BeanCostService {
    ...
    public Result getCost(String type) {
        ...
        if (type.startsWith("qrcode")) {
            ...
            BeanCost beanCost = initProject.beanCosts.stream().filter(s -> s.getType().equals(strs1[0])).collect(Collectors.toList()).get(0);
            count += beanCost.getBean();
            if (strs1.length > 1 && !"".equals(strs1[1])) {
                String[] strs2 = strs1[1].split(";");
                BeanCost beanCost2 = initProject.beanCosts.stream().filter(s -> s.getType().equals(strs2[0])).collect(Collectors.toList()).get(0);
                int fileSize_M = Integer.valueOf(strs2[1]);
                count += (fileSize_M + 5) / 5 * beanCost2.getBean();
            }
        } else {
            BeanCost beanCost = initProject.beanCosts.stream().filter(s -> s.getType().equals(type)).collect(Collectors.toList()).get(0);
            count = beanCost.getBean();
        }
