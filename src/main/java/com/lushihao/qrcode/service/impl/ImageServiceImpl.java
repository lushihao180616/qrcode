...
public class ImageServiceImpl implements ImageService {
    ...
    public String addWaterMark(WaterMark waterMark) {
        try {
            ...
            //获得水印
            BufferedImage waterMarkImage = ImageIO.read(new FileInputStream(projectBasicInfo.getBusinessUrl() + "\\" + waterMark.getBusinessCode() + "\\waterMark.png"));
            ...
            if (bg.getWidth() * 1.2 < bg.getHeight()) {
                ...
                Font font = new Font("微软雅黑", Font.PLAIN, (int) (coverHeight * 0.7));
                coverG2.setFont(font);              //设置字体
                coverG2.setColor(Color.WHITE); //根据图片的背景设置水印颜色
                ...
                coverG2.drawString(phone, (float) ((name.length()) * (coverHeight * 0.7) + (coverHeight * 0.65)), (float) (coverHeight * 0.7));
                coverG2.drawString("地址：『" + nowBusiness.getAddress() + "』", (float) ((name.length() + phone.length() - 5) * (coverHeight * 0.7) + (coverHeight * 1.45)), (float) (coverHeight * 0.7));
            }
            coverG2.dispose();
